...
subscribeToEvent('OrderPlaced', (event) => handleOrderPlaced(event, notifyOrderPlacedUseCase));
subscribeToEvent('PaymentCompleted', (event) => handlePaymentCompleted(event, notifyPaymentCompletedUseCase));
subscribeToEvent('ShipmentDispatched', (event) => handleShipmentDispatched(event, notifyShipmentDispatchedUseCase));
subscribeToEvent('ShipmentDelivered', (event) => handleShipmentDelivered(event, notifyShipmentDeliveredUseCase));
app.listen(PORT, () => {
    console.log(`[Notifications Domain] Server running on http://localhost:${PORT}`);
});
